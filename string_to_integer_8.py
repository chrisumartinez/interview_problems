...
# Read in next the characters until the next non-digit charcter or the
...
# Since -91283472332 is less than the lower bound of
#  the range [-231, 231 - 1], the final result is clamped to -231 = -2147483648.


def myAtoi(s: str) -> int:
    # number set to check each character
    ...
    if s[index] == "-":
        # if the sign is negative, parse the string into an int:
        for letter in range(index + 1, len(s)):
            if s[letter] in num_set:
                result += s[letter]
            else:
                if s[letter] == "+" or s[letter] == "-":
                    pass
                else:
                    break
        print("result: ", result)
        if (0 - int(result)) < INT_MIN:
            return INT_MIN
        else:
            return 0 - int(result)
    else:
        # check if the item is not in the num_set:
        if s[index] not in num_set:
            # its an invalid input, return 0:
            return 0
...
# print(myAtoi(String3))
# print(myAtoi(String4))
print(myAtoi(String6))
# print(myAtoi(String6))
# print(myAtoi(String7))